...
import {
    getAuth, sendEmailVerification, signInWithEmailAndPassword
} from 'firebase/auth';
...
export default function StudentLogin({navigation, setUser}) {
    const [loading, setLoading] = useState(false)
    const [values, setValues] = useState({email: '', password: ''})
    const [eateryLogin, setEateryLogin] = useState([])

    const onChange = obj => setValues({...values, ...obj})
    const gotoAccount = () => navigation.navigate('StudentAccount')
